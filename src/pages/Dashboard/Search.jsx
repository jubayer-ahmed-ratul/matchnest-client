...
export default function Search() {
  ...
  const inputClass = "border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 bg-white";
  ...
  return (
    <div>
      ...
      <form onSubmit={handleFilter} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6 flex flex-wrap gap-3 items-end">
        ...
        <button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold px-5 py-2 rounded-xl transition">
          Search
        </button>
      </form>
          ...
          {profiles.map((p) => (
            <div key={p._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition overflow-hidden">
              ...
              <div className="p-4">
                ...
                {p.profession && <p className="text-sm text-gray-500 flex items-center gap-1"><HiOutlineBriefcase className="w-4 h-4" />{p.profession}</p>}
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    disabled={sentIds.includes(p._id)}
                    onClick={() => handleInterest(p._id)}
                    className={`flex-1 text-xs py-1.5 rounded-lg transition font-semibold ${
                      sentIds.includes(p._id)
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-6">
          <button disabled={page === 1} onClick={() => setPage(page - 1)}
            className="text-sm border border-gray-200 px-4 py-2 rounded-xl hover:border-orange-400 hover:text-orange-500 transition disabled:opacity-40">
            ← Prev
          </button>
          <span className="text-sm border border-gray-200 px-4 py-2 rounded-xl text-gray-500">{page} / {totalPages}</span>
