...
export const generateReceipt = async ({ plan, amount, currency, email, date, id }) => {
  ...
  const rows = [["Plan", details.name], ["Amount", "$" + amount + " " + (currency || "USD")], ["Status", "Paid"], ["Method", "Card (Stripe)"]];
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  rows.forEach(function(item, i) {
    ...
    doc.setTextColor(item[0] === "Status" ? 34 : 30, item[0] === "Status" ? 150 : 30, item[0] === "Status" ? 80 : 30);
    doc.text(item[1], 100, y);
    doc.setTextColor(30, 30, 30);
  });
  doc.setFontSize(9);
  doc.setTextColor(120, 120, 120);
